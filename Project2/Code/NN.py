...
class NeuralNetwork:
    def __init__(
        ...
        self.layers = np.round(
            np.linspace(self.n_features, self.n_categories, n_hidden_layers + 2)
        )[1:-1]
    ...
    def create_biases_and_weights(self):
        ...
        for layer in range(1, self.n_hidden_layers):
            self.w.append(
                np.random.randn(self.layers[layer - 1], self.layers[layer])
            )  # W
...
if skleearn_compare == True:
    ...
    dnn = MLPClassifier(
        hidden_layer_sizes=neurons_layers,
        activation="logistic",
        alpha=lmd_NN,
        learning_rate_init=eta_NN,
        max_iter=epochs_NN,
    )
