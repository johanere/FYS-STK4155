...
for i in range(0,7):
    for j in range(0,3):
        square_real=Terrain2.template[500*i:500*(i+1),500*j:500*(j+1)]
        square_pred=Terrain2.predicted_terrain[500*i:500*(i+1),500*j:500*(j+1)]
        deviationaver=deviationaver+abs(np.mean(np.ravel(square_real))-np.mean(np.ravel(square_pred)))
